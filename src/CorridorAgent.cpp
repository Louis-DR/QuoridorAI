...
uint8_t CorridorAgent::minimax(QuoridorBoard game, int depth, bool is_first_player)
{
    return 0;
}
...
uint8_t CorridorAgent::evaluationHeuristic(QuoridorBoard game)
{
    ...
    uint8_t position_node_player2 = graph.coords2NodeId(player2.position_x, player2.position_y);
    uint8_t min_distance_player1 = graph.getMinDistance(position_node_player1, true);
    uint8_t min_distance_player2 = graph.getMinDistance(position_node_player2, false);
